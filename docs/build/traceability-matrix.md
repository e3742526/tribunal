...
| REQ-005 | P0 | Exact quorum, consensus, dissent and arbitration | exhaustive decision tables pass | ADR-0003 | domain tables | partial |
| REQ-006 | P0 | Durable external state and resumable lifecycle | faulted mandatory writes cannot report success | ADR-0002 | storage/application suites | partial |
...
| REQ-009 | P0 | Bounded output, calls, tokens and time | each cap reaches typed terminal behavior | architecture | cap tables | partial |
| REQ-010 | P0 | Findings ledger and decision memory | cross-run lifecycle and rulings persist | ADR-0003 | storage round trips + exact match | partial |
...
| REQ-012 | P1 | Codex, Claude, Agy and OpenAI-compatible reviewers | golden requests plus bounded smoke | ADR-0004 | adapter suite + local HTTP E2E | partial |
...
| REQ-016 | P1 | Plaintext, Markdown, DOCX and PDF review | extraction fixtures produce anchored packets | ADR-0007 | packet/DOCX tests; doctor PDF | partial |
| REQ-017 | P1 | Markdown/HTML reports, bench and status TUI | outputs escape content and consume RunSnapshot | architecture | application/TUI source | partial |
| REQ-018 | P1 | Complete CLI and JSON contracts | every specified command has a real handler | io-contract | CLI surface + review E2E | partial |
| REQ-019 | P1 | Rebranded CI/release/security/docs | clean-checkout check and archive smoke | ADR-0001 | Gate 8 | partial |
