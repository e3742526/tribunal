...
| Claude output hardening | `go test ./internal/tagteam/` (`adapters_part02_test.go`, 10 tests) | pass | `adapters_part02_test.go` | Claude envelope `is_error`/`error_*` surfacing, embedded-JSON candidate recovery (fenced blocks, decoy objects, unmatched braces) across worker/review contracts | Real Claude prose variants are sampled, not exhaustive |
| Claude invocation serialization | `go test ./internal/tagteam/` (`invocation_lock_test.go`, 6 tests) | pass | `invocation_lock_test.go` | flock-based cross-process lock acquire/release, live-holder wait, fail-closed timeout/cancellation, state-root derivation, and a re-exec multi-process mutual-exclusion test | Windows PID-file fallback path is compile-checked, not executed, in CI |
