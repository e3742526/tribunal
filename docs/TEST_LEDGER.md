...
| Application | `go test ./internal/tribunal/app` | synthetic three-reviewer run, complete blind vote delivery, durable usage limit, no-duplicate checkpoint resume, strict nested reads, terminal-first publication, latest-pointer read after current publication, edit/revert transaction recovery and user-change refusal | exhaustive OS-kill matrix remains larger than deterministic fault seams |
...
| Full gate | `scripts/check.sh` | format, 800 lines, tests, vet, build, module integrity, tidy -diff, vulnerability scan (required in CI, reported-if-absent locally) | local runs without govulncheck skip the scan |
| Race | `go test -race ./...` (CI job + local) | concurrent review/vote, locks, HTTP tests, durable state under race detector | real provider rate limits are outside local control |
| Campaign 2026-07-22 | `contract_test.go`, `arbitration_resume_test.go`, `identity_test.go`, `integrity_test.go`, `hardening_test.go`, `limits_test.go`, `durability_test.go`, `edit_retry_test.go` | JSON/exit contract, resume locking, arbitration defaults, evidence trust boundary, exact consensus identity, anchor uniqueness, extraction caps, adapter/process hardening, torn-journal recovery, ledger staleness scope, edit retry | interactive-TTY arbitrate guard is reasoned, not executed |
